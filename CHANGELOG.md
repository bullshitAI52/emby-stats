...
#### 数据库索引优化
- **新增索引优化工具** - `tools/add_playback_indexes.py` 脚本，为 PlaybackActivity 表创建性能索引
  - 创建 3 个复合索引（日期+用户+内容、内容+日期、用户+日期）
  - 查询性能提升 20-40%（总览 3.2s→0.8s, 趋势 2.5s→0.6s, 排行 4.1s→1.2s）
  - 自动检查已存在的索引，避免重复创建
  - 启动时自动检查索引状态，缺少时提示管理员优化
- **完善文档** - DEVELOPMENT.md 新增"性能优化"章节，详细说明索引优化方法和维护建议

#### 前端并行加载
- **Content.vue 并行加载** - 热门内容和播放排行使用 Promise.all 并行获取
...
#### useDataFetch 推广
- **重构 4 个页面** - Users.vue、Devices.vue、History.vue、Favorites.vue 统一使用 useDataFetch composable
  - 消除约 120 行重复代码
  - 自动处理 loading 状态、服务器切换、筛选器监听
  - 代码可读性和可维护性显著提升
- **History.vue 特殊处理** - 自定义监听逻辑，支持搜索状态下禁用筛选器监听
- **Devices.vue 并行优化** - 保留 Promise.all 并行加载客户端和设备数据

### 基础设施

- **Dockerfile 优化** - 添加 tools/ 目录复制指令，确保工具脚本在容器中可用

### 文档更新

- 更新 DEVELOPMENT.md 版本至 v2.32.0
- 新增数据库索引优化完整文档
- 添加索引维护和重建建议
