...
│   │   │       ├── PageHeader.vue    # 页面标题（v2.32.5）
│   │   │       ├── EmptyState.vue    # 空状态（v2.32.5）
│   │   │       ├── LoadingState.vue  # 加载状态（v2.32.5）
│   │   │       ├── PasswordField.vue # 密码字段（v2.32.6）
│   │   │       ├── PosterCard.vue    # 海报卡片
...
│   │   │   ├── useLoading.ts         # 加载状态
│   │   │   ├── useApi.ts             # API 调用封装
│   │   │   ├── usePosterUrl.ts       # 海报URL处理（v2.32.6）
│   │   │   └── useSearch.ts          # 搜索过滤（v2.32.6）
│   │   ├── plugins/
...
<PageHeader title="设备统计" subtitle="了解客户端和设备的使用情况">
  <v-btn color="primary">刷新</v-btn>
</PageHeader>
```

#### EmptyState.vue（v2.32.5 新增）
- 统一的空状态组件
- 显示提示信息
...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `message` | string | `'暂无数据'` | 提示消息 |
| `type` | string | `'info'` | 提示类型（info/warning/error/success） |

**使用示例：**
```vue
<EmptyState message="暂无统计数据" />
<EmptyState message="未找到内容" type="warning" />
```

#### LoadingState.vue（v2.32.5 新增）
- 统一的加载状态组件
- 支持多种预设布局
...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `preset` | string | `'custom'` | 预设布局类型 |
| `minHeight` | string | `'200px'` | spinner 模式最小高度 |
| `size` | number | `48` | spinner 尺寸 |

**预设布局：**
| 预设名 | 说明 |
|--------|------|
| `overview` | 4列统计卡片 + 2个图表 |
| `chart-table` | 1个图表 + 1个表格 |
| `dual-chart-dual-table` | 2个图表 + 2个表格 |
| `stats-table` | 3列统计卡片 + 1个表格 |
| `table` | 单个表格 |
| `spinner` | 居中加载指示器 |
| `custom` | 自定义插槽内容 |
...
<LoadingState v-if="loading" preset="spinner" />
...
### v2.32.6 (2025-12-18) - 🔧 可复用组件与Composables优化

- 新增 `usePosterUrl` composable：统一海报URL处理（server_id、尺寸参数）
- 新增 `useSearch` composable：统一搜索过滤逻辑
- 新增 `PasswordField.vue` 组件：密码显示/隐藏切换
- 新增 `VALIDATION_RULES` 常量：统一表单验证规则（required、positiveNumber、numeric等）
- 全局样式统一：`.card-header`、`.poster-thumbnail`、`.rank-badge`、`.ranking-number`
- 重构 History、Content、ContentDetail、Favorites、Login、Report、Tools 页面
- 删除约 200 行重复代码（样式、验证规则、海报处理逻辑）

---

### v2.32.5 (2025-12-18) - 🧩 页面结构组件提取

- 新增 `PageHeader.vue` 页面标题组件（7个页面应用）
- 新增 `EmptyState.vue` 空状态组件（8个页面应用）
- 新增 `LoadingState.vue` 加载状态组件（支持多种预设布局）
- 重构 Overview、Users、Devices、History、Content、Favorites、Report 页面
- 删除约 300 行重复样式代码（page-header、page-title、page-subtitle）
- 统一页面标题和加载/空状态的展示方式
